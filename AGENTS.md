...
```bash
cd /Users/christ/Workspace/git/xianyu/QianuHub-Xianyu-Helper
...
```bash
cd /Users/christ/Workspace/git/xianyu/QianuHub-Xianyu-Helper/frontend
npm install
npm run build
```
...
- Windows installs the `QianuHubXianyuHelper` Windows Service and starts `xianyu-tray.exe` for the current
...
- macOS registers `com.qianuhub.xianyu-helper.server` and `com.qianuhub.xianyu-helper.tray` LaunchAgents.
  The app is `/Applications/QianuHub闲鱼助手/QianuHub闲鱼助手.app`, and the tray executable is named
  `QianuHub闲鱼助手` with `LSUIElement=true` so it does not appear in the Dock.
- Linux packages are architecture-specific tar archives. `install.sh` must run as root on a native matching
  architecture, installs `qianuhub-xianyu-helper.service`, and keeps data in `/var/lib/qianuhub-xianyu-helper`.
...
```bash
runtime_app="$PWD/dist/macos/pkgroot-arm64/Applications/QianuHub闲鱼助手/QianuHub闲鱼助手.app"
mkdir -p /tmp/qianuhub-local-data
"$runtime_app/Contents/Helpers/xianyu-server" \
  -addr 127.0.0.1:59189 \
  -workdir /tmp/qianuhub-local-data \
