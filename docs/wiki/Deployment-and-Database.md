...
| --- | --- | --- | --- |
| Linux amd64/arm64 | Extract and run `./install.sh` as root | `qianuhub-xianyu-helper.service` | `http://127.0.0.1:59188` |
| Windows | Run the Inno Setup installer | `QianuHubXianyuHelper` | `http://127.0.0.1:59188` |
| macOS arm64/amd64 | Run the package for the matching architecture | `com.qianuhub.xianyu-helper.server` | `http://127.0.0.1:59188` |

Windows and macOS packages also install an independent tray/menu-bar controller. It shows checking, starting, healthy, and stopping states; starts, stops, and restarts the backend; opens the log directory; and waits for confirmed shutdown before exiting. During Windows installation, the interactive user receives permission to query, start, and stop this service, so normal tray actions do not repeatedly show UAC. Changing service configuration or deleting the service still requires administrator rights. Linux has no desktop tray; use systemd and `journalctl -u qianuhub-xianyu-helper.service`.

Data and log locations are `C:\ProgramData\QianuHubXianyuHelper` on Windows, `~/Library/Application Support/QianuHubXianyuHelper` and `~/Library/Logs/QianuHubXianyuHelper` on macOS, and `/var/lib/qianuhub-xianyu-helper` and `/var/log/qianuhub-xianyu-helper` on Linux. Select a CPU-matching arm64 or amd64 package on macOS. Install Linux packages on the same architecture as the host; the installer does not use QEMU.
...
Windows and macOS tray menus can start, stop, or restart the backend, open the management page, and open the log directory. Choosing Exit Tray stops the backend first, preventing an unmanaged background process. The Windows installer configures the service’s minimum control permission once. A normal interactive user can later query, start, and stop `QianuHubXianyuHelper` from the tray without another UAC prompt; changing configuration, deleting the service, and controlling other services still require administrator rights.

- Windows: `C:\ProgramData\QianuHubXianyuHelper\logs\server.log`
- macOS: `~/Library/Logs/QianuHubXianyuHelper/server.log`, with tray output in `tray.log` in the same directory
- Linux: `/var/log/qianuhub-xianyu-helper/server.log`; `journalctl -u qianuhub-xianyu-helper.service` is also available
