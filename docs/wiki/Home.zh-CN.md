...
Windows 安装包注册 `QianuHubXianyuHelper` 服务；macOS 安装包分别提供 Apple Silicon（arm64）和
Intel（amd64）版本，并注册 `com.qianuhub.xianyu-helper.server` 与
`com.qianuhub.xianyu-helper.tray` 两个 LaunchAgent。Linux 安装包按 amd64/arm64 分发，执行
`sudo ./install.sh` 后注册 `qianuhub-xianyu-helper.service`。三种独立安装包都在构建阶段准备匹配架构
...
- [订单管理](订单管理.zh-CN)
- [发货模板](发货模板.zh-CN)
- [自动化规则](自动化发货与回复.zh-CN)
- [在线聊天与账号自动任务](在线聊天与账号自动任务.zh-CN)
- [批量铺货](批量铺货.zh-CN)
- [通知设置](AI、通知与运维.zh-CN)
- [系统与AI](系统设置.zh-CN)
...
这些 Markdown 文件位于仓库 `docs/wiki/`，是 Wiki 的唯一维护来源；英文页面使用默认文件名，中文页面使用
`.zh-CN` 后缀，`_Sidebar.md` 是 Wiki 导航。`main` 分支发生变更后，`.github/workflows/sync-wiki.yml` 会自动把这些文件镜像到 GitHub
