...
import { createItem,deleteItem,getItemDetail,getItems,publishItem,recommendPublishCategory,syncItemsFromAccount,updateItem } from './items/api';
...
import { changePassword,getSystemSettings,updateLoginCredentials,updateSystemSettings } from './settings/api';
...
test('updateSystemSettings separates sensitive values into explicit commands', /* 当前回调验证敏感设置三态命令请求体。 */ async () => {
  ...
  stubContractFetch(fetchMock);
  await updateSystemSettings({ smtp_password: '' });
  // payload 是普通设置与敏感命令分离后的请求体。
  const payload = JSON.parse(fetchMock.mock.calls[0][1].body);
  expect(payload.values).toEqual({});
  expect(payload.secrets).toEqual({ smtp_password: { action: 'clear' } });
});
...
test('Settings 配置和凭据 API 转发外部取消信号', async () => {
  // fetchMock 验证 Settings 的读取和凭据保存共享取消控制能力。
  const fetchMock = vi.fn()
  ...
  expect(fetchMock).toHaveBeenNthCalledWith(2, '/api/v1/session', expect.objectContaining({ signal: expect.any(AbortSignal) }));
  expect(fetchMock).toHaveBeenNthCalledWith(3, '/api/v1/session/credentials', expect.objectContaining({ signal: expect.any(AbortSignal) }));
} /* 测试回调验证：Settings 配置和凭据 API 转发外部取消信号。 */);
...
test('列表 API 接受历史命名包裹对象并保持具名 UI 结果', async () => {
  // fetchMock 模拟订单、商品和卡密接口各自的历史字段名称。
  const fetchMock = vi.fn()
...
test('updateShippingRule posts every matching card action before confirm shipment', async () => {
  ...
  expect(JSON.parse(body.actions[0].config_json)).toEqual({ delay_override: false });
  expect(JSON.parse(body.actions[1].config_json)).toEqual({ delay_override: true });
...
test('updateShippingRule preserves text actions while editing card variants', async () => {
  ...
  await updateShippingRule({
    ...
    item_id: 'item-1',
    trigger_type: 'order_paid',
     variants: [{ card_id: 8, delivery_count: 1, enabled: true }],
...
// 商品同步和类目推荐 API 使用版本化兼容入口。
const runVersionedItemCategoryAPITest = async () => {
  // fetchMock 是商品同步和类目推荐请求的测试替身。
  const fetchMock = vi.fn()
    .mockResolvedValueOnce(jsonResponse({ success: true }))
...
test('item sync and category APIs use versioned compatibility routes', runVersionedItemCategoryAPITest);
...
const runVersionedSettingsCardNotificationAPITest = async () => {
  ...
  expect(fetchMock).toHaveBeenNthCalledWith(3, '/api/v1/cards', expect.objectContaining({ method: 'GET' }));
  expect(fetchMock).toHaveBeenNthCalledWith(4, '/api/v1/cards', expect.objectContaining({ method: 'POST' }));
  expect(fetchMock).toHaveBeenNthCalledWith(5, '/api/v1/cards/1', expect.objectContaining({ method: 'PUT' }));
  expect(fetchMock).toHaveBeenNthCalledWith(6, '/api/v1/cards/1', expect.objectContaining({ method: 'DELETE' }));
  expect(fetchMock).toHaveBeenNthCalledWith(7, '/api/v1/cards/1/details', expect.objectContaining({ method: 'GET' }));
  expect(fetchMock).toHaveBeenNthCalledWith(8, '/api/v1/cards/batch', expect.objectContaining({ method: 'POST', body: expect.any(FormData) }));
  expect(fetchMock).toHaveBeenNthCalledWith(9, '/api/v1/cards/1/append-data', expect.objectContaining({ method: 'POST' }));
  expect(fetchMock).toHaveBeenNthCalledWith(10, '/api/v1/notifications/channels', expect.objectContaining({ method: 'GET' }));
  expect(fetchMock).toHaveBeenNthCalledWith(11, '/api/v1/notifications/channels', expect.objectContaining({ method: 'POST' }));
  expect(fetchMock).toHaveBeenNthCalledWith(12, '/api/v1/notifications/channels/1', expect.objectContaining({ method: 'PUT' }));
  expect(fetchMock).toHaveBeenNthCalledWith(13, '/api/v1/notifications/channels/1', expect.objectContaining({ method: 'DELETE' }));
  expect(fetchMock).toHaveBeenNthCalledWith(14, '/api/v1/notifications/messages', expect.objectContaining({ method: 'GET' }));
  expect(fetchMock).toHaveBeenNthCalledWith(15, '/api/v1/notifications/accounts/acc1/bindings', expect.objectContaining({ method: 'POST' }));
  expect(fetchMock).toHaveBeenNthCalledWith(16, '/api/v1/notifications/messages/1', expect.objectContaining({ method: 'DELETE' }));
  expect(fetchMock).toHaveBeenNthCalledWith(17, '/api/v1/notifications/messages/account/acc1', expect.objectContaining({ method: 'DELETE' }));
  expect(fetchMock).toHaveBeenNthCalledWith(18, '/api/v1/notifications/accounts/acc1/bindings', expect.objectContaining({ method: 'GET' }));
  ...
  expect(fetchMock).toHaveBeenNthCalledWith(20, '/api/v1/notifications/channels/1/test', expect.objectContaining({ method: 'POST' }));
};
