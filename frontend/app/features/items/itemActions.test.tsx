...
const useItemActionsHarness = () => {
  ...
  const options: ItemActionsOptions = { selectedAccount, setSelectedAccount, setItems, loadItems, loadShippingRules, onConfigureDelivery };
...
describe('useItemActions', /* 当前回调验证商品普通操作、发布和定位边界。 */ () => {
  ...
  test('定位成功写入普通发布发货地并收口状态', /* 当前回调验证普通发布定位动作的成功分支。 */ async () => {
    ...
    await act(/* 当前回调加载普通发布发货地。 */ async () => hook.result.current.actions.locateForPublish());
  ...
  test('关闭普通发布弹窗后忽略晚到的浏览器定位与地点回调', /* 当前回调验证弹窗关闭会撤销定位请求的界面所有权。 */ async () => {
    ...
    act(/* beginLocateAction 启动尚未返回的定位流程。 */ () => { void hook.result.current.actions.locateForPublish(); });
