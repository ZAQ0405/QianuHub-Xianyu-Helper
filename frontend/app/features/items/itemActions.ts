...
// ItemActionsOptions 描述商品动作协调器依赖的列表状态。
...
export interface ItemActionsState {
  ...
  // locateForPublish 获取普通发布使用的发货地。
  locateForPublish: () => Promise<void>;
}
...
export const useItemActions = ({ selectedAccount, setSelectedAccount, setItems, loadItems, loadShippingRules, onConfigureDelivery }: ItemActionsOptions): ItemActionsState => {
  // loading 表示商品同步请求是否正在执行。
  ...
  const handlePublishItem = useCallback(/* publishAction 执行普通商品发布。 */ async () => {
    ...
    try {
      ...
      const totalQuantity = Number(publishForm.quantity);
      // result 保存平台发布接口返回的商品信息。
  ...
  // locateForPublish 使用浏览器定位并加载普通发布发货地。
  const locateForPublish = useCallback(/* locateAction 获取发布发货地。 */ async () => {
    // cookieId 保存当前定位请求使用的账号。
    const cookieId = publishForm.cookie_id;
    if (!cookieId) return alert('请先选择发布账号');
