...
// Rules - 自动化规则
const normalizeShippingRules = (rules: any[]): ShippingRule[] => rules.filter(/* 当前回调过滤已下线的旧规则类型。 */ (item: any) => item.trigger_type === 'order_paid' || item.trigger_type === 'review_missing_timeout').map(/* 当前回调用于处理集合元素或接口响应。 */ (item: any) => ({
        id: String(item.id),
        name: item.name || '',
...
const orderAutomationActions = (triggerType: string, actions: AutomationAction[]) => {
    ...
    // confirmAction 保留用户配置的确认发货文案，同时由保存逻辑统一补齐动作顺序。
    const confirmAction = actions.find(/* 当前回调用于查找确认发货动作。 */ action => action.action_type === 'confirm_shipment');
    return [
      ...deliveryActions,
      ...others.map(/* 当前回调用于处理集合元素或接口响应。 */ (action, index) => ({ ...action, sort_order: deliveryActions.length + index + 1 })),
      { ...confirmAction, action_type: 'confirm_shipment' as const, enabled: true, sort_order: deliveryActions.length + others.length + 1 },
    ];
};
...
export const updateShippingRule = async (rule: Partial<ShippingRule>): Promise<OperationResponse | MutationIDResponse> => {
    ...
    // preservedConfirmAction 保留规格模式下已有的确认发货文案。
    const preservedConfirmAction = (rule.actions || []).find(/* 当前回调用于查找确认发货动作。 */ action => action.action_type === 'confirm_shipment');
    ...
    const baseActions: AutomationAction[] = rule.variants && rule.variants.length > 0
      ? [...rule.variants.map(/* 当前回调用于处理集合元素或接口响应。 */ (variant, index) => ({
		  ...
		  })), ...preservedNonCardActions, ...(preservedConfirmAction ? [preservedConfirmAction] : [])]
