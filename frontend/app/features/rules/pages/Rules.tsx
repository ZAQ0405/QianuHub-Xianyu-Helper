...
import { accentClasses,accountLabel,actionSummary,buildReviewConfig,cardActionsForTrigger,confirmShipmentMessage,isDeliveryCardReady,needsAllItemsConfirmation,statusPill,triggerMeta,triggerOrder,withAllItemsConfirmation } from '../utils';
...
const Rules: React.FC<RulesProps> = ({ initialDeliveryTarget, onDeliveryTargetHandled }) => {
  ...
  const {
    ...
    handleAutomationItemChange, updateVariant, updateConfirmShipmentMessage, appendDeliveryContent, toggleRepublishItem, handleSaveAutomationRule, handleDeleteAutomation,
  ...
  return (
    <div className="page-stack min-w-0 animate-fade-in">
      <div className="flex flex-col xl:flex-row justify-between xl:items-end gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-2 font-medium">系统通知卡片只进入自动化判断；买家消息进入关键词或默认回复。</p>
        </div>
                    ...
                    {selectedRuleItem && currentTrigger !== 'review_missing_timeout' && (
                      <div className="mt-4 rounded-2xl bg-gray-50 border border-gray-100 p-4">
                        <div className="flex flex-wrap items-center gap-2 mb-2">
                          ...
                          <span className="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-50 text-emerald-700">单规格商品 · 按订单数量发卡</span>
                        </div>
                        <p className="text-xs leading-5 text-gray-500">
                          买家付款后，系统按订单购买数量发送卡密；资料商品售出后可继续重新上架。
                        </p>
                      </div>
                    )}
                  </section>
                    ...
                    <section className="bg-white rounded-3xl border border-gray-100 p-5">
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="font-black text-gray-900">发货库存</h4>
                          </div>
                      ...
                      <div className="space-y-3">
                        {displayVariants.map((variant, index) => (/* 当前回调处理集合中的单个元素。 */
                          <div
                            ...
                            className="rules-delivery-grid grid min-w-0 grid-cols-1 gap-3 items-end rounded-2xl border border-gray-200 p-4"
                          >
                            <div>
                      ...
                      {currentTrigger === 'order_paid' && (
                        <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50/70 p-4">
                          <label className="block text-sm font-black text-gray-900 mb-2">确认发货文案（可选）</label>
                          <textarea
                            value={confirmShipmentMessage(editingAutomationRule.actions)}
                            onChange={/* 当前回调更新确认发货文案。 */ event => updateConfirmShipmentMessage(event.target.value)}
                            placeholder="例如：资料已发送，请查收"
                            className="w-full ios-input px-3 py-2.5 rounded-xl h-24 resize-none"
                          />
                          <p className="text-xs leading-5 text-gray-600 mt-2">确认发货成功后会单独发送给买家；卡密内容仍按上方发货库存发送。</p>
                        </div>
                      )}
                      {currentTrigger === 'order_paid' && editingAutomationRule.item_id && (
                        <div className="mt-4 rounded-2xl border border-blue-100 bg-blue-50/70 p-4">
                          <label className="flex items-start gap-3 cursor-pointer">
                            <input
                              type="checkbox"
                              checked={editingAutomationRule.actions?.some(/* 当前回调查找启用的自动上架动作。 */ action => action.action_type === 'republish_item' && action.enabled) === true}
                              onChange={/* 当前回调切换付款后的商品自动上架动作。 */ event => toggleRepublishItem(event.target.checked)}
                              className="mt-1 accent-brand"
                            />
                            <span>
                              <span className="block text-sm font-black text-gray-900">付款成功并发送资料后，自动确认发货并重新上架原商品</span>
                              <span className="block text-xs leading-5 text-gray-600 mt-1">仅适用于单库存、无多规格电子资料。系统先调用闲鱼虚拟发货接口完成订单状态，再恢复原商品上架，保留原商品 ID，不创建新商品。</span>
                            </span>
                          </label>
                        </div>
                      )}
            ...
            <div className="modal-header">
              <div className="flex items-center justify-between w-full">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">关键词未处理时，才会使用默认回复。</p>
                </div>
                <button
