...
import { buildConfirmShipmentMessage,buildReviewConfig,cardActionsForTrigger,defaultRuleName,emptyVariant,parseJSONObject,shouldReplaceGeneratedName,triggerMeta,withAllItemsConfirmation } from './utils';
...
export interface RuleActionsState {
  ...
  // updateConfirmShipmentMessage 更新确认发货动作的虚拟发货文案。
  updateConfirmShipmentMessage: (text: string) => void;
  // appendDeliveryContent 追加一行发货规格。
  appendDeliveryContent: () => void;
  // toggleRepublishItem 切换付款后自动重新上架动作。
  toggleRepublishItem: (enabled: boolean) => void;
...
export const useRuleActions = ({
  ...
  // updateConfirmShipmentMessage 更新确认发货动作的虚拟发货文案。
  const updateConfirmShipmentMessage = useCallback(/* confirmMessageAction 更新确认发货文案。 */ (text: string) => {
    if (!editingAutomationRule) return;
    // actions 保存当前规则动作的独立副本，避免直接修改 React 草稿引用。
    const actions = (editingAutomationRule.actions || []).slice();
    // confirmIndex 保存确认发货动作在当前列表中的位置。
    const confirmIndex = actions.findIndex(/* 当前回调查找确认发货动作。 */ action => action.action_type === 'confirm_shipment');
    if (confirmIndex >= 0) {
      actions[confirmIndex] = buildConfirmShipmentMessage(actions[confirmIndex], text);
    } else {
      actions.push({ action_type: 'confirm_shipment', enabled: true, sort_order: actions.length + 1, message_template: text });
    }
  ...
  const appendDeliveryContent = useCallback(/* appendVariantAction 追加规则规格。 */ () => {
    ...
    setEditingAutomationRule({
      ...editingAutomationRule,
      variants: [...displayVariants, emptyVariant()],
    });
  }, [displayVariants, editingAutomationRule]);

  // toggleRepublishItem 根据用户勾选状态增删付款后自动重新上架动作。
  const toggleRepublishItem = useCallback(/* toggleRepublishAction 切换自动上架选项。 */ (enabled: boolean) => {
    if (!editingAutomationRule) return;
    // actions 保存当前规则动作的独立副本，避免直接修改 React 草稿引用。
    const actions = (editingAutomationRule.actions || []).filter(/* 当前回调移除旧的自动上架动作。 */ action => action.action_type !== 'republish_item');
    if (enabled) {
      actions.push({ action_type: 'republish_item', enabled: true, sort_order: actions.length + 1 });
    }
    setEditingAutomationRule({ ...editingAutomationRule, actions });
  }, [editingAutomationRule]);
  ...
  const handleSaveAutomationRule = useCallback(/* saveRuleAction 保存自动化规则。 */ async () => {
    ...
    if (trigger !== 'review_missing_timeout') {
      if (!variants.length || variants.some(/* 当前回调校验卡密库存是否已选择。 */ variant => !variant.card_id)) return alert('请选择发货卡密库存');
    }
    if (trigger === 'review_missing_timeout') {
      // text 保存求评价动作的文案。
    ...
    // saveActions 保存实际提交的动作列表。
    const saveActions = editingAutomationRule.actions?.length ? editingAutomationRule.actions : cardActionsForTrigger(trigger, editingAutomationRule.card_group_id || 0);
    // saveVariants 保存归一化后的发货规格。
    const saveVariants = trigger === 'review_missing_timeout' ? [] : variants.map(/* 当前回调归一化单规格发货内容。 */ variant => ({ ...variant, delivery_count: Math.max(1, Number(variant.delivery_count) || 1), enabled: variant.enabled !== false }));
    setAutomationSubmitState(startRuleSubmission(automationSubmitState));
    // succeeded 记录保存是否成功。
    let succeeded = false;
    try {
      await updateShippingRule({ ...editingAutomationRule, trigger_type: trigger, name: (editingAutomationRule.name || '').trim() || defaultRuleName(trigger, selectedRuleItem?.item_title || editingAutomationRule.item_id || ''), config_json: trigger === 'review_missing_timeout' ? buildReviewConfig(editingAutomationRule.config_json) : (editingAutomationRule.config_json || '{}'), actions: trigger === 'order_paid' ? saveActions : (editingAutomationRule.actions?.length ? editingAutomationRule.actions : cardActionsForTrigger(trigger, saveVariants[0]?.card_id || editingAutomationRule.card_group_id || 0)), variants: saveVariants });
  ...
  }, [automationSubmitState, editingAutomationRule, loadAutomationRules, loadReferenceData, selectedRuleItem]);
  ...
  return {
    ...
    editingReplyRule, setEditingReplyRule, defaultForm, setDefaultForm, selectedRuleItem, currentTrigger,
    currentMeta, reviewConfig, displayVariants, buildAutomationDraft, openAutomationRule, openNewAutomationRule,
    handleTriggerChange, handleAutomationItemChange, updateVariant, updateConfirmShipmentMessage, appendDeliveryContent, toggleRepublishItem, handleSaveAutomationRule,
