...
export const useSettings = (): UseSettingsResult => {
  ...
  useEffect(/* 当前回调同步 React 副作用和资源生命周期。 */ () => {
    loadSettings();
    return /* 当前回调处理异步资源释放。 */ () => { requestController.current?.abort(); };
  }, [loadSettings]);
  ...
  return {
    ...
    credentials, requestStatus, loadSettings,
    handleSave, handleCredentialsSave, setSettings, setShowCaptchaSecret,
