...
func NewRuntimeBundle(store *db.Store, bm *browser.Manager, logger *slog.Logger) (*RuntimeBundle, error) {
	...
	// itemPublisher 复用单商品发布端口，统一处理自动上架的凭证锁、Cookie 写回和平台重试。
	itemPublisher := NewItemPublishPort(store, func() mtop.Client { return mtop.NewClient() }, logger,
		func(ctx context.Context, cookieID, cookieValue string) {
			// sender、ok 分别表示在线账号发送器及其是否可用。
			if sender, ok := manager.Sender(cookieID); ok && sender != nil {
				sender.UpdateCookie(cookieValue)
			}
		}, func(ctx context.Context, cookieID string, err error) bool {
			return runtimeAdapter.RecoverExpiredCredential(ctx, cookieID)
		})
	// itemRepublisher 在资料发送完成后负责把原商品按同一商品 ID 重新上架。
	itemRepublisher := NewItemRepublisher(itemPublisher)
