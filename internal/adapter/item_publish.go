...
// itemRelistClient 定义原商品重新上架所需的窄平台能力，避免扩大通用 mtop.Client 契约。
type itemRelistClient interface {
	ReshelfItem(context.Context, string, string) (*mtop.AccountTaskResult, error)
}

// RelistItem 调用原商品同 ID 的重新上架接口，不创建新商品或修改本地商品绑定。
func (p *ItemPublishPort) RelistItem(ctx context.Context, cookieID, itemID string) error {
	return p.relistItem(ctx, strings.TrimSpace(cookieID), strings.TrimSpace(itemID), true)
}

// relistItem 执行一次原商品重新上架，并在凭证恢复成功时最多重试一次。
func (p *ItemPublishPort) relistItem(ctx context.Context, cookieID, itemID string, allowRetry bool) error {
	if p == nil || p.store == nil || p.store.Cookies == nil {
		return errors.New("商品重新上架存储未初始化")
	}
	if cookieID == "" || itemID == "" {
		return errors.New("商品重新上架缺少账号或商品ID")
	}
	// unlock 保护账号凭证快照读取与远端调用后的提交复核。
	unlock := p.store.LockAccountCredentials(cookieID)
	// latest、loadErr 保存加锁后读取的平台凭证视图。
	latest, loadErr := p.store.Cookies.GetCookiePlatformRuntimeData(ctx, cookieID)
	if loadErr != nil || !hasStoredCredential(latest) {
		unlock()
		return errors.New("账号凭证已变化，请重试")
	}
	// requestCtx、cancel 控制原商品重新上架的最长执行时间。
	requestCtx, cancel := context.WithTimeout(ctx, 90*time.Second)
	defer cancel()
	// mtopCtx、cookieSession 保存带 Cookie 快照的平台调用上下文。
	mtopCtx, cookieSession := withCookieSnapshot(requestCtx, latest)
	// initialValue、initialMetadata 保存远端调用前的凭证版本。
	initialValue, initialMetadata := latest.Value, latest.MetadataJSON
	unlock()
	// relister、ok 表示当前客户端是否提供原商品重新上架能力。
	relister, ok := p.mtopClient().(itemRelistClient)
	if !ok {
		return errors.New("当前平台客户端不支持原商品重新上架")
	}
	// result、callErr 保存原商品重新上架结果及平台调用错误。
	result, callErr := relister.ReshelfItem(mtopCtx, latest.Value, itemID)
	// publishResult 仅借用统一会话提交逻辑保存 Cookie，不代表创建了新商品。
	var publishResult *mtop.PublishItemResult
	if result != nil {
		publishResult = &mtop.PublishItemResult{UpdatedCookies: result.UpdatedCookies}
	}
	// runtimeCookie、persistErr 保存响应 Cookie 提交后的运行时同步值和错误。
	runtimeCookie, persistErr := p.persistPublishSession(ctx, itemapp.PublishInput{UserID: latest.UserID, CookieID: cookieID}, initialValue, initialMetadata, cookieSession, publishResult, callErr)
	if runtimeCookie != "" && p.updateRunningCookie != nil {
		p.updateRunningCookie(ctx, cookieID, runtimeCookie)
	}
	if callErr != nil {
		if persistErr != nil {
			callErr = errors.Join(callErr, fmt.Errorf("保存重新上架响应 Cookie: %w", persistErr))
		}
		if allowRetry && mtop.IsSessionExpiredErr(callErr) && p.recoverExpired(ctx, cookieID, callErr) {
			return p.relistItem(ctx, cookieID, itemID, false)
		}
		p.recoverExpired(ctx, cookieID, callErr)
		return callErr
	}
	if persistErr != nil {
		return fmt.Errorf("保存重新上架响应 Cookie: %w", persistErr)
	}
	if result == nil || !result.Success {
		return errors.New("原商品重新上架未返回成功结果")
	}
	return nil
}
