...
// ActionRepublishItem 表示付款并完成资料发送后自动重新上架原商品。
const ActionRepublishItem = "republish_item"
...
func (s *RuleService) normalize(ctx context.Context, userID int64, draft RuleDraft, allowedDisabledTemplateIDs map[int64]int64) (RuleInput, error) {
	...
	if draft.TriggerType != TriggerOrderPaid && draft.TriggerType != TriggerReviewMissingTimeout {
	...
	// actions 和 flags 在付款后重新上架场景中补齐确认发货，保证规则不会因前端或旧调用方漏传动作而停留在未发货状态。
	actions, flags = ensurePaidRepublishConfirmation(draft.TriggerType, actions, flags)
	if flags.hasRepublishItem && draft.ItemID == "" {
		return RuleInput{}, errors.New("自动上架动作必须绑定具体商品")
	}
...
type ruleActionFlags struct {
	...
	hasAdjustPrice bool
	// hasRepublishItem 表示是否存在启用的付款后重新上架动作。
	hasRepublishItem bool
}
...
func (s *RuleService) normalizeDraftActions(ctx context.Context, userID int64, triggerType string, draftActions []ActionDraft, allowedDisabledTemplateIDs map[int64]int64) ([]ActionInput, ruleActionFlags, error) {
	...
	for index, draftAction := range draftActions {
		...
		case ActionConfirmShipment:
			draftAction.MessageTemplate = strings.TrimSpace(draftAction.MessageTemplate)
		...
		case ActionSendTemplate, ActionAdjustPrice:
			// 个人版不再提供模板发货和订单改价；旧动作即使通过接口提交也不能重新进入规则。
			return nil, flags, errors.New("该自动化动作已移除，请使用卡密发货、确认发货或重新上架")
		case ActionSendText:
		...
		case ActionRepublishItem:
			if triggerType != TriggerOrderPaid {
				return nil, flags, errors.New("自动上架动作仅支持付款后规则")
			}
			flags.hasRepublishItem = flags.hasRepublishItem || enabled
		default:
