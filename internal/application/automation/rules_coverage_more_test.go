...
// TestRuleServiceNormalizesSupportedActionKinds 验证个人版保留动作的默认值、启用状态与持久化配置。
func TestRuleServiceNormalizesAllActionKinds(t *testing.T) {
	...
	actions, flags, err := service.normalizeDraftActions(context.Background(), 7, TriggerOrderPaid, []ActionDraft{
		{ActionType: ActionConfirmShipment, MessageTemplate: "  资料已发送，请查收  ", Enabled: &disabled},
	...
	if len(actions) != 3 || flags.hasConfirmShipment || !flags.hasSendCard || !flags.hasSendText || flags.hasAdjustPrice {
		t.Fatalf("动作结果错误: actions=%+v flags=%+v", actions, flags)
	}
	if actions[0].Enabled || actions[0].MessageTemplate != "资料已发送，请查收" || actions[0].DeliveryCount != 1 || actions[1].SortOrder != 3 || actions[2].SortOrder != 3 {
...
func TestRuleServiceRejectsActionValidationBranches(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "template trigger", service: NewRuleService(&ruleRepositoryFake{}, &ruleOwnershipFake{}), trigger: TriggerOrderCreated, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1}, want: "动作已移除"},
		{name: "template id", service: NewRuleService(&ruleRepositoryFake{}, &ruleOwnershipFake{}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate}, want: "动作已移除"},
		{name: "template capability", service: NewRuleService(&ruleRepositoryFake{}, &ruleOwnershipFake{}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1}, want: "动作已移除"},
		{name: "template lookup", service: NewRuleService(&ruleRepositoryFake{}, &ruleTemplateOwnershipFake{ruleOwnershipFake: &ruleOwnershipFake{}, templateErr: templateError}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1}, want: "动作已移除"},
		{name: "template disabled", service: NewRuleService(&ruleRepositoryFake{}, &ruleTemplateOwnershipFake{ruleOwnershipFake: &ruleOwnershipFake{}, template: TemplateInfo{}}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1}, want: "动作已移除"},
		{name: "template binding", service: NewRuleService(&ruleRepositoryFake{}, &ruleTemplateOwnershipFake{ruleOwnershipFake: &ruleOwnershipFake{}, template: TemplateInfo{Enabled: true, Keys: []string{"key"}}}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1}, want: "动作已移除"},
		{name: "template custom", service: NewRuleService(&ruleRepositoryFake{}, &ruleTemplateOwnershipFake{ruleOwnershipFake: &ruleOwnershipFake{}, template: TemplateInfo{Enabled: true, CustomKeys: []string{"name"}}}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendTemplate, DeliveryTemplateID: 1, CustomVariables: map[string]string{}}, want: "动作已移除"},
		{name: "text message", service: NewRuleService(&ruleRepositoryFake{}, &ruleOwnershipFake{}), trigger: TriggerOrderPaid, action: ActionDraft{ActionType: ActionSendText}, want: "发送文本动作必须填写文案"},
...
// TestRuleServiceRejectsTemplateAction 验证已移除的模板动作不能重新进入规则。
func TestRuleServiceNormalizesTemplateAction(t *testing.T) {
	...
	// _, _, err 保存模板动作被拒绝的结果。
	_, _, err := service.normalizeDraftActions(context.Background(), 7, TriggerOrderPaid, []ActionDraft{{
	...
	if err == nil || !containsRuleError(err, "动作已移除") {
		t.Fatalf("模板动作应被拒绝: %v", err)
	}
}

// TestRuleServiceRejectsRemovedTemplateOnUpdate 验证更新规则时也不能保留已移除模板动作。
func TestRuleServiceAllowsOnlyExistingDisabledTemplateOnUpdate(t *testing.T) {
	...
	if _, err := service.NormalizeForUpdate(context.Background(), 7, 9, draft); err == nil || !containsRuleError(err, "动作已移除") {
		t.Fatalf("既有模板动作应被拒绝：%v", err)
	}
...
func TestRuleServiceRejectsInvalidNormalizeInputs(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "empty actions", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderPaid}, want: "至少需要一个自动化动作"},
		{name: "config", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderPaid, ConfigJSON: "[]", Actions: []ActionDraft{{ActionType: ActionSendText, MessageTemplate: "x"}}}, want: "规则配置"},
	}
