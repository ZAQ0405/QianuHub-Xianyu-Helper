...
// TestRuleServiceRejectsRemovedAdjustPrice 验证个人版不再接受订单改价动作。
func TestRuleServiceRejectsAdjustPriceWhenAIEnabled(t *testing.T) {
	...
	// err 是已删除触发器被拒绝时返回的输入错误。
	if _, err := service.Normalize(context.Background(), 7, draft); err == nil || !strings.Contains(err.Error(), "不支持的触发类型") {
		t.Fatalf("订单改价规则应被拒绝: %v", err)
	}
}
...
func TestRuleServiceRejectsInvalidAction(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "unknown action", ownership: &ruleOwnershipFake{}, draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderPaid, Actions: []ActionDraft{{ActionType: "unknown"}}}, want: "不支持的动作"},
	}
...
// TestRuleServiceNormalizeOrderCreatedAdjustPrice 验证拍下未付款改价规则已被移除。
func TestRuleServiceNormalizeOrderCreatedAdjustPrice(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "不支持的触发类型") {
		t.Fatalf("拍下改价规则应被拒绝: %v", err)
	}
}
...
func TestRuleServiceRejectsInvalidOrderCreatedRules(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "missing adjust price", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionSendText, MessageTemplate: "x"}}}, want: "不支持的触发类型"},
		{name: "send card forbidden", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionSendCard, CardID: 1}, {ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1"}`}}}, want: "不支持的触发类型"},
		{name: "confirm shipment forbidden", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionConfirmShipment}, {ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1"}`}}}, want: "不支持的触发类型"},
		{name: "missing price", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionAdjustPrice, ConfigJSON: `{}`}}}, want: "不支持的触发类型"},
		{name: "bad price format", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1.234"}`}}}, want: "不支持的触发类型"},
		{name: "zero price", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"0"}`}}}, want: "不支持的触发类型"},
		{name: "too high price", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderCreated, Actions: []ActionDraft{{ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1000000.01"}`}}}, want: "不支持的触发类型"},
		{name: "adjust price on paid trigger", draft: RuleDraft{CookieID: "a", TriggerType: TriggerOrderPaid, Actions: []ActionDraft{{ActionType: ActionSendCard, CardID: 1}, {ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1"}`}}}, want: "动作已移除"},
		{name: "adjust price on review trigger", draft: RuleDraft{CookieID: "a", TriggerType: TriggerReviewMissingTimeout, Actions: []ActionDraft{{ActionType: ActionSendText, MessageTemplate: "x"}, {ActionType: ActionAdjustPrice, ConfigJSON: `{"target_price":"1"}`}}}, want: "动作已移除"},
	}
