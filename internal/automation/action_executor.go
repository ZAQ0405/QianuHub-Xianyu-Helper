...
type automationActionExecutor struct {
	...
	// republisher 返回构造期固定的商品重新发布能力。
	republisher func() ItemRepublisher
...
func (e *automationActionExecutor) executeActionWithProof(ctx context.Context, task Task, action db.AutomationAction, proof shipmentDeliveryProof) (actionExecutionResult, error) {
	...
	case ActionConfirmShipment:
		// confirmationTask 表示用户已在自动化规则中明确选择确认发货，因此不受账号默认开关阻断。
		confirmationTask := task
		confirmationTask.ForceConfirmShipment = true
		// confirmationMessage 保存规则中配置并按订单上下文渲染后的确认发货文案。
		confirmationMessage := renderTemplate(strings.TrimSpace(action.MessageTemplate), task)
		// err 保存先确认订单状态再发送补充文案的结果；文案通过买家聊天发送，避免只写入平台凭证而不可见。
		if err := e.confirmShipmentWithProof(ctx, confirmationTask, proof); err != nil {
			return actionExecutionResult{}, err
		}
		if strings.TrimSpace(confirmationMessage) == "" {
			return actionExecutionResult{}, nil
		}
		// sendErr 表示确认发货成功后向买家发送补充文案的结果。
		if sendErr := e.sendText(ctx, task, confirmationMessage); sendErr != nil {
			if errors.Is(sendErr, ErrMessageNotSent) {
				return actionExecutionResult{}, sendErr
			}
			// reviewProof 保存结果未知的补充文案，人工核对后可按原文补发。
			reviewProof := shipmentDeliveryProof{messages: []db.AutomationDeliveryMessage{{Kind: "text", Content: confirmationMessage}}}
	...
	case ActionRepublishItem:
		// republisher 保存构造期固定的原商品重新上架能力，避免自动化中心直接依赖平台实现。
		var republisher ItemRepublisher
		if e.republisher != nil {
			republisher = e.republisher()
		}
		if republisher == nil {
			return actionExecutionResult{}, fmt.Errorf("自动上架能力未初始化")
		}
		if task.ItemID == "" || task.OrderID == "" {
			return actionExecutionResult{}, fmt.Errorf("自动上架缺少商品或订单ID")
		}
		// err 保存原商品重新上架的外部结果，失败时必须按不确定动作交给人工核对。
		if err := republisher.RepublishItem(ctx, RepublishItemRequest{AccountID: task.AccountID, ItemID: task.ItemID, OrderID: task.OrderID}); err != nil {
			return actionExecutionResult{}, uncertainAction(err)
		}
