...
func isDeliveryAction(action db.AutomationAction) bool {
	// 当前个人版只支持卡密库存发货；模板发货属于已移除功能，不得再进入付款计划。
	return action.ActionType == ActionSendCard
}
...
func (actionPlanner) plan(task Task, actions []db.AutomationAction) []db.AutomationAction {
	...
	if task.TriggerType == TriggerOrderPaid {
		// action 表示当前遍历过程中的动作
		for /* action 表示当前付款规则中的候选自动上架动作。 */ _, action := range actions {
			if action.Enabled && isDeliveryAction(action) && actionMatchesOrderSpec(task, action) {
				out = append(out, action)
			}
		}
		// action 表示付款后负责把订单状态推进为已发货的确认动作。
		for /* action 表示当前付款规则中的确认发货动作。 */ _, action := range actions {
			if action.Enabled && action.ActionType == ActionConfirmShipment {
				out = append(out, action)
			}
		}
		// action 表示确认发货完成后恢复原商品上架状态的动作。
		for /* action 表示当前付款规则中的原商品重新上架动作。 */ _, action := range actions {
			if action.Enabled && action.ActionType == ActionRepublishItem {
				out = append(out, action)
			}
		}
		return out
	}
	if task.TriggerType != TriggerReviewMissingTimeout {
		// 已移除的拍下改价和评价赠品不再生成任何执行计划。
		return out
	}
	// action 表示当前超时邀评规则中的启用文本动作。
	for _, action := range actions {
		if action.Enabled && action.ActionType == ActionSendText {
			out = append(out, action)
		}
	}
	return out
