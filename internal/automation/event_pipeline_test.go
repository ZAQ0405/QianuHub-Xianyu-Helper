...
// TestActionPlannerConfirmsBeforeRepublish 验证电子资料先完成订单发货状态，再重新上架原商品。
func TestActionPlannerConfirmsBeforeRepublish(t *testing.T) {
	// task 保存最小付款事实。
	task := Task{TriggerType: TriggerOrderPaid, ItemID: "item-1", OrderID: "order-1"}
	// plan 保存包含发卡、确认发货和自动上架的动作顺序。
	plan := (actionPlanner{}).plan(task, []db.AutomationAction{
		{ID: 3, ActionType: ActionRepublishItem, Enabled: true},
		{ID: 1, ActionType: ActionConfirmShipment, Enabled: true},
		{ID: 2, ActionType: ActionSendCard, Enabled: true, ConfigJSON: `{}`},
	})
	// got 保存规划后动作的标识顺序。
	if got := []int64{plan[0].ID, plan[1].ID, plan[2].ID}; !reflect.DeepEqual(got, []int64{2, 1, 3}) {
		t.Fatalf("电子资料付款后动作顺序=%v，want [2 1 3]", got)
	}
}
