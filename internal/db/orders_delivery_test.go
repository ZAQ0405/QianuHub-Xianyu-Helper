...
// TestCards_DeleteCleansDeletedRuleReference 验证商品被删除后，卡密删除会清理无效规则引用，但不会绕过有效规则保护。
func TestCards_DeleteCleansDeletedRuleReference(t *testing.T) {
	// s、cleanup 用于本次流程后续判断的s、cleanup
	s, cleanup := newTestDB(t)
	defer cleanup()
	// ctx 用于本次流程后续判断的ctx
	ctx := context.Background()
	// uid、cookieID 用于本次流程后续判断的用户和账号范围。
	uid, cookieID := seedAccount(t, s)

	// staleCardID、err 保存已删除商品规则使用的卡密组。
	staleCardID, err := s.Cards.Create(ctx, &CardFull{Name: "失效商品卡密", Type: "text", TextContent: "stale", Enabled: true, UserID: uid})
	if err != nil {
		t.Fatal(err)
	}
	// staleRuleID、err 保存模拟商品删除后遗留的停用规则。
	staleRuleID, err := s.Automation.Create(ctx, makeAutomationRule(cookieID, uid, "deleted-item", "paid", false, 100,
		AutomationActionInput{ActionType: "send_card", CardID: staleCardID, Enabled: true}))
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.DB.ExecContext(ctx, `UPDATE automation_rules SET deleted_at=CURRENT_TIMESTAMP WHERE id=?`, staleRuleID); err != nil {
		t.Fatal(err)
	}
	if err := s.Cards.Delete(ctx, staleCardID); err != nil {
		t.Fatalf("已删除商品的卡密应可清理: %v", err)
	}
	var staleRuleCount int
	if err := s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM automation_rules WHERE id=?`, staleRuleID).Scan(&staleRuleCount); err != nil || staleRuleCount != 0 {
		t.Fatalf("失效规则未清理: count=%d err=%v", staleRuleCount, err)
	}

	// liveCardID、err 保存仍被有效规则引用的卡密组。
	liveCardID, err := s.Cards.Create(ctx, &CardFull{Name: "有效商品卡密", Type: "text", TextContent: "live", Enabled: true, UserID: uid})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := s.Automation.Create(ctx, makeAutomationRule(cookieID, uid, "live-item", "paid", true, 100,
		AutomationActionInput{ActionType: "send_card", CardID: liveCardID, Enabled: true})); err != nil {
		t.Fatal(err)
	}
	if err := s.Cards.Delete(ctx, liveCardID); !errors.Is(err, ErrCardInUse) {
		t.Fatalf("有效规则引用的卡密应继续受保护: %v", err)
	}
}
