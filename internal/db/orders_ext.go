...
func (o *Orders) FindLatestPendingByChat(ctx context.Context, cookieID, chatID, buyerID, itemID string) (*Order, error) {
	...
	// args 保存 where 条件对应的绑定参数；unknown 覆盖先收到简化付款通知、尚未完成订单详情同步的本地订单。
	args := []any{accountID, sessionID, "pending_ship", "paid", "2", "pending_delivery", "partial_success", "partial_pending_finalize", "unknown"}
...
// Delete 删除卡券，并清理已删除商品留下的无效自动化规则引用。
func (c *Cards) Delete(ctx context.Context, cardID int64) error {
	// tx 保证清理孤儿规则、检查剩余引用和删除卡券在同一事务内完成。
	...
	// err 清理已删除商品对应的规则；有进行中、人工核对、结果未知或可重试任务的规则必须保留。
 ...
 WHERE deleted_at IS NOT NULL
   AND id IN (
       SELECT DISTINCT r.id
        ...
        WHERE (a.card_id=? OR EXISTS (
                  SELECT 1 FROM automation_action_template_bindings b
                   WHERE b.action_id=a.id AND b.card_id=?
              ))
   )
   AND NOT EXISTS (
	...
	// references 只统计清理后仍存在的动作和模板变量引用。
	var references int
...
SELECT
    (SELECT COUNT(*)
      ...
      WHERE a.card_id=?)+
    (SELECT COUNT(*)
       FROM automation_action_template_bindings b
       JOIN automation_rule_actions a ON a.id=b.action_id
       JOIN automation_rules r ON r.id=a.rule_id
      WHERE b.card_id=?)`, cardID, cardID).Scan(&references); err != nil {
		return err
	}
	if references > 0 {
		return ErrCardInUse
