...
func TestOpenAPIRemainingVersionedSuccessResponses(t *testing.T) {
	...
	t.Run("removed-feature-routes", testOpenAPIRemovedFeatureRoutes)
}

// testOpenAPIRemovedFeatureRoutes 验证已删除功能的版本化入口不会重新暴露。
func testOpenAPIRemovedFeatureRoutes(t *testing.T) {
	// srv、_、cleanup 分别是路由服务、测试存储和资源释放函数。
	srv, _, cleanup := newTestServer(t)
	defer cleanup()
	// handler 是当前版本完整路由树，登录仅用于排除认证状态对结果的影响。
	handler := srv.Router()
	sessionCookie := loginHelper(t, handler)
	// cases 覆盖 AI、发货模板、批量铺货和多规格入口。
	cases := []struct {
		method string
		path   string
	}{
		{method: http.MethodPost, path: "/api/v1/settings/ai-models"},
		{method: http.MethodGet, path: "/api/v1/settings/ai-reply"},
		{method: http.MethodGet, path: "/api/v1/delivery-templates"},
		{method: http.MethodPost, path: "/api/v1/items/publish-batches/preview"},
		{method: http.MethodPut, path: "/api/v1/items/account/item/multi-spec"},
	}
	for _, routeCase := range cases {
		// request 是当前已删除入口的验证请求。
		request := httptest.NewRequest(routeCase.method, routeCase.path, strings.NewReader(`{}`))
		request.AddCookie(sessionCookie)
		// recorder 捕获路由树对已删除入口的响应。
		recorder := httptest.NewRecorder()
		handler.ServeHTTP(recorder, request)
		if recorder.Code != http.StatusNotFound && recorder.Code != http.StatusMethodNotAllowed {
			t.Errorf("已删除入口仍可访问 method=%s path=%s status=%d", routeCase.method, routeCase.path, recorder.Code)
		}
	}
}
