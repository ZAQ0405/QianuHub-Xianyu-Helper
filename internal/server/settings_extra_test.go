...
func TestListAIModels(t *testing.T) {
	assertRemovedServerEndpoint(t, http.MethodGet, "/ai-models")
	return
...
func TestSetAIReplyBadJSON(t *testing.T) {
	assertRemovedServerEndpoint(t, http.MethodGet, "/ai-reply-settings")
	return
	// srv、cleanup 用于本次流程后续判断的srv、cleanup
	srv, _, cleanup := newTestServer(t)
	defer cleanup()
...
func TestGetAIReplyExistingAccountWithoutConfigReturnsDefault(t *testing.T) {
	assertRemovedServerEndpoint(t, http.MethodGet, "/ai-reply-settings/acc1")
	return
	// srv、cleanup 用于本次流程后续判断的srv、cleanup
	srv, _, cleanup := newTestServer(t)
	defer cleanup()
