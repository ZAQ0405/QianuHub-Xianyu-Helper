...
func (c *ClientImpl) accountTaskRequestOnce(ctx context.Context, cookiesStr, endpoint, api, version string, data map[string]any, referer string) (*accountTaskResponse, string, error) {
	...
	if api == "mtop.idle.pc.idleitem.editDetail" || api == "mtop.idle.pc.idleitem.edit" {
		query.Set("spm_cnt", "a21ybx.publish.0.0")
		query.Set("spm_pre", "a21ybx.publish.0.0")
		if api == "mtop.idle.pc.idleitem.editDetail" {
			query.Set("log_id", "xianyu_item_edit_detail")
		} else {
			query.Set("log_id", "xianyu_item_edit")
		}
	}
