...
APP_DIR="$ROOT_DIR/Applications/QianuHub闲鱼助手"
APP="$APP_DIR/QianuHub闲鱼助手.app"
PACKAGE_PATH="$DIST_DIR/QianuHub-Xianyu-Helper-$VERSION-$ARCH.pkg"
UNSIGNED_PACKAGE_PATH="$DIST_DIR/.QianuHub-Xianyu-Helper-$VERSION-$ARCH.unsigned.pkg"
...
cp "$BIN_DIR/xianyu-tray" "$APP/Contents/MacOS/QianuHub闲鱼助手"
cp "$SCRIPT_DIR/uninstall.command" "$APP_DIR/卸载 QianuHub闲鱼助手.command"
cp "$SCRIPT_DIR/com.qianuhub.xianyu-helper.server.plist.template" "$APP/Contents/Resources/"
cp "$SCRIPT_DIR/com.qianuhub.xianyu-helper.tray.plist.template" "$APP/Contents/Resources/"
...
chmod 0755 "$APP/Contents/MacOS/QianuHub闲鱼助手" "$APP/Contents/Helpers/xianyu-server" "$APP_DIR/卸载 QianuHub闲鱼助手.command"
...
if [ -n "${MACOS_SIGNING_IDENTITY:-}" ]; then
  ...
  sign_code "$APP/Contents/MacOS/QianuHub闲鱼助手"
  sign_code "$APP"
...
pkgbuild \
  ...
  --identifier com.qianuhub.xianyu-helper \
  --version "$VERSION" \
  --install-location / \
