...
$resourceSpecs = @(
    @{ Package = 'cmd\server'; Output = 'rsrc'; Manifest = 'cli'; Description = 'QianuHub Xianyu Helper server'; Filename = 'xianyu-server.exe' },
    @{ Package = 'cmd\tray'; Output = 'rsrc'; Manifest = 'gui'; Description = 'QianuHub Xianyu Helper tray'; Filename = 'xianyu-tray.exe' }
)
...
foreach ($spec in $resourceSpecs) {
    ...
    & go run $winresTool simply `
        ...
        --icon $iconPath `
        --product-name 'QianuHub Xianyu Helper' `
