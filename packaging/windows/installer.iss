...
#define AppDataDir "{commonappdata}\QianuHubXianyuHelper"
...
OutputBaseFilename=QianuHub-Xianyu-Helper-Setup
...
[CustomMessages]
english.ProductName=QianuHub Xianyu Helper
chinesesimplified.ProductName=QianuHub闲鱼助手
chinesetraditional.ProductName=QianuHub闲鱼助手
...
[Registry]
Root: HKCU; Subkey: "Software\Microsoft\Windows\CurrentVersion\Run"; ValueType: string; ValueName: "QianuHubXianyuHelperTray"; ValueData: """{app}\xianyu-tray.exe"""; Flags: uninsdeletevalue
...
begin
  Result := '-NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "' +
    ...
    '" -WorkDir "' + ExpandConstant('{commonappdata}\QianuHubXianyuHelper') +
...
begin
  ...
  Parameters := ServiceScriptParameters(
    ...
    ' -FailureLogPath "' +
      ExpandConstant('{commonappdata}\QianuHubXianyuHelper\logs\service-registration-error.log') +
    '"';
  ...
  begin
    ...
    SuppressibleMsgBox(
      ...
      '详细原因已写入 %ProgramData%\QianuHubXianyuHelper\logs\service-registration-error.log。',
      mbError, MB_OK, IDOK);
    Abort;
  end;
